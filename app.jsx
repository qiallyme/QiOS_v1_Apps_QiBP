...
export default function BPTracker() {
  ...
  useEffect(() => {
    ...
    if (video) {
      ...
      // After video plays for a few seconds, fade it out
      const fadeTimer = setTimeout(() => {
        setVideoFading(true);
        setTimeout(() => setShowSplash(false), 500);
      }, 3000); // Play for 3 seconds then fade
  ...
  useEffect(() => {
    ...
    // Only overwrite default if the user specifically saved a different one previously
  ...
  // Fetch Sheet Data when entering dashboard
  useEffect(() => {
    if (view === 'dashboard' && sheetUrl) {
      fetchSheetData();
    }
  }, [view]);
  ...
  // Robust CSV parser that handles quoted fields (e.g. "Dec 1, 2024")
  ...
  const fetchSheetData = async () => {
    ...
    // Auto-fix for Edit links if user changes it back accidentally
    if (sheetUrl.includes('/edit') && !sheetUrl.includes('output=csv')) {
    ...
    try {
      ...
      // Simple CSV parsing assuming headers in row 0
      const headers = rows[0].map(h => h.toLowerCase().trim());
      const sysIdx = headers.findIndex(h => h.includes('sys'));
      const diaIdx = headers.findIndex(h => h.includes('dia'));
      const pulseIdx = headers.findIndex(h => h.includes('pulse') || h.includes('heart'));
      const dateIdx = headers.findIndex(h => h.includes('date') || h.includes('time') || h.includes('created') || h.includes('timestamp'));

      // Fallback indices if headers not found
      const sI = sysIdx > -1 ? sysIdx : 0;
      const dI = diaIdx > -1 ? diaIdx : 1;
      const pI = pulseIdx > -1 ? pulseIdx : 2;
      const tI = dateIdx > -1 ? dateIdx : 3;

      const parsedData = rows.slice(1)
        .filter(r => r.length > 1 && r[sI] && r[dI] && !isNaN(parseInt(r[sI])))
        .map(r => ({
          systolic: parseInt(r[sI]),
          diastolic: parseInt(r[dI]),
          pulse: parseInt(r[pI]) || 0,
          timestamp: r[tI] || ''
        }))
        // Note: We don't reverse here if we want the chart to be chronological left-to-right based on array index
        // But usually sheets append new data to bottom. 
        // We will reverse for display list, but keep original order for chart if needed.
        .reverse(); 

      setSheetData(parsedData);
    } catch (err) {
      console.error(err);
      setSheetError('Connection failed. Ensure the Google Sheet link is correct.');
    } finally {
  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-900 font-sans flex flex-col max-w-md mx-auto shadow-2xl overflow-hidden relative">
      
      {/* Splash screen with intro video */}
      {showSplash && (
        <div className={`absolute inset-0 z-50 flex items-center justify-center bg-slate-900 overflow-hidden transition-opacity duration-500 ${videoFading ? 'opacity-0' : 'opacity-100'}`}>
          <video
            ref={videoRef}
          ...
          >
            ...
            <div className="absolute inset-0 flex flex-col items-center justify-center text-white">
              <div className="flex items-center gap-3 mb-4">
                ...
                <div>
                  <div className="text-lg font-semibold tracking-tight">BP Tracker</div>
                  <div className="text-xs text-slate-300">Powered by QiAlly</div>
                </div>
              </div>
            </div>
          </video>
      ...
      <main className="flex-1 -mt-8 px-4 pb-20 z-20 relative overflow-y-auto">
              ...
              <div className="grid grid-cols-2 gap-4 mb-6">
                <div className="space-y-2">
                   ...
                   <button 
                     ...
                     className={`w-full p-3 rounded-xl border flex flex-col items-center justify-center gap-1 transition-all ${
                       aspirin 
                ...
                <div className="space-y-2">
                   ...
                   <div className={`w-full p-3 rounded-xl border flex flex-col items-center justify-center gap-2 transition-all ${
                       water > 0
              ...
              <div>
                ...
                <div className="grid grid-cols-2 gap-3">
                  ...
                  <button 
                    ...
                    className={`p-4 rounded-xl border flex flex-col items-center justify-center gap-2 transition-all ${
                      symptoms.includes('Short of Breath') 
        ...
        {view === 'dashboard' && (
          <div className="space-y-4 animate-in fade-in slide-in-from-right-4 duration-300">
             ...
             {!sheetUrl ? (
               <div className="text-center py-8 bg-blue-50 rounded-xl p-6 border border-blue-100">
                 <BarChart3 className="w-12 h-12 mx-auto mb-3 text-blue-300" />
                 <h4 className="font-bold text-blue-900 mb-2">Connect Google Sheets</h4>
                 <p className="text-sm text-blue-700 mb-4">
                   To see live trends, publish your Google Sheet as a CSV and paste the link in Settings.
                 </p>
                 <button onClick={() => setView('settings')} className="bg-blue-600 text-white px-4 py-2 rounded-lg text-sm font-medium">
                   Configure Dashboard
                 </button>
               </div>
             ) : sheetError ? (
                <div className="bg-red-50 text-red-600 p-4 rounded-xl text-sm border border-red-100">
                  <div className="flex gap-2 items-center font-bold mb-1"><AlertCircle className="w-4 h-4"/> Connection Error</div>
                  {sheetError}
                </div>
             ) : (
               <>
                 ...
                 <div className="bg-white p-4 rounded-xl shadow-sm border border-slate-100">
                   <h4 className="text-xs font-bold uppercase text-slate-400 mb-4 tracking-wider">Recent Trend</h4>
                 ...
                 {/* Recent Sheet List */}
                 <h4 className="text-xs font-bold uppercase text-slate-400 mt-4 tracking-wider">Sheet Data</h4>
                 <div className="space-y-2">
                    {sheetData.slice(0, 5).map((log, i) => (
                      <div key={i} className="bg-white p-3 rounded-lg border border-slate-100 flex justify-between items-center text-sm">
                        ...
                        <span className="text-slate-400 text-xs">{log.timestamp || 'Unknown Date'}</span>
                      </div>
                    ))}
                 </div>
               </>
                 ...
                 <div key={i} className="bg-white p-4 rounded-xl shadow-sm border border-slate-100 flex flex-col items-start gap-2">
                   <div className="flex justify-between w-full">
                     <div className="flex items-baseline gap-1">
                       ...
                       <span className={`text-xs ml-2 font-medium px-2 py-0.5 rounded-full ${
                         log.category === 'Normal' ? 'bg-green-100 text-green-700' :
